...
const ExpedientAdd = ({ onAddExpedient }) => {
  ...
  const handleSubmit = () => {
    if (nombres && primerApellido && areaDependencia && funcionEjercida) {
      ...
      onAddExpedient(newExpedient);
      clearForm();
      setMessage('¡Expediente agregado con éxito! ¡Eres un crack!');
      setTimeout(() => setMessage(''), 3000);
    } else {
      setMessage('Por favor, llena los campos obligatorios. ¡No te hagas el despistado!');
    }
  };
  ...
  return (
    <div className="flex flex-col items-center flex-grow bg-gray-50 p-6">
      <div className="bg-white p-10 rounded-lg shadow-xl w-full max-w-4xl border border-gray-200">
        <h2 className="text-3xl font-bold text-gray-800 mb-8 text-center">Agregar Nuevo Expediente</h2>
        {message && (
        ...
        <div className="mb-8 p-6 border border-gray-200 rounded-lg shadow-sm">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center">
            <button className="bg-gray-200 text-gray-700 py-3 px-6 rounded-md text-lg font-semibold hover:bg-gray-300 transition-all duration-300 shadow-md">
              Subir archivo SNTE (PDF)
            </button>
            <button className="bg-gray-200 text-gray-700 py-3 px-6 rounded-md text-lg font-semibold hover:bg-gray-300 transition-all duration-300 shadow-md">
              Subir archivos adicionales (PDF)
            </button>
          </div>
        </div>
        ...
        <div className="flex justify-end space-x-4">
          ...
          <button
            ...
            className="bg-green-600 text-white py-3 px-6 rounded-md text-xl font-semibold hover:bg-green-700 transition-all duration-300 shadow-lg transform hover:scale-105"
          >
            Guardar Expediente
          </button>
        </div>
